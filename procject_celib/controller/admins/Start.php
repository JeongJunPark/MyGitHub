		...
		if ($this->Admin_model->verifyUser()) {
			...
			$data = array();
  			while ( $week['Sat_ymd'] >= $tmp_dt ) {
        		$data[$tmp_dt] = array();
				$query = " select count(id) as count from tours where house_id=4 and date_format(created_at, '%Y-%m-%d') = '".$tmp_dt."'";
        		...
        		$query = " select ts.state, count(ts.id) as count from tour_state as ts
						...
						where t.house_id = 4 and date_format(ts.created_at, '%Y-%m-%d') = '".$tmp_dt."' group by ts.state";
