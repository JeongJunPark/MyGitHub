...
class Beds extends MY_Controller {
	...
	{ 
        ...
        $totalcount = $this->BedModel->totalcount();
        $total_pages = (int) ($totalcount/$per_page);
        if($total_pages==0) $total_pages =1;
        if( $total_pages <  ($totalcount/$per_page) ) $total_pages++;
        $data['total_pages'] = $total_pages;
        $data['data'] = $this->BedModel->get_list($page, $order, $per_page);

		$this->load->view('admin/header');
		$this->load->view('admin/beds/list', $data);
